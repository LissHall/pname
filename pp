...
case "$1" in
    ...
    *)
        ...
        echo "  pp add NAME [PATH]   # Add new path (uses current dir if PATH omitted)"
        echo "  pp ls                # List all paths"
        echo "  pp cd NAME           # Change to specified path (prints path in quotes)"
        echo "  pp rm NAME           # Delete specific path record"
        echo "  pp rm _all           # Delete ALL stored paths (requires confirmation)"
        exit 1
        ;;
esac
